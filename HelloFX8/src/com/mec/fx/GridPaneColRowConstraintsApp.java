...
public class GridPaneColRowConstraintsApp extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		for(int row = 0; row < 3; ++row){
			for(int col = 0; col < 3; ++col){
				Button b = new Button(String.format(Msg.get(this, "buttonNamePattern"), col, row));
				root.add(b,  col, row);
			}
		}
		...
		//Set the percent width for the second column to 30% 
		//and the horizontal alignment to center
		ColumnConstraints cc2 = new ColumnConstraints();
		cc2.setPercentWidth(35);
		cc2.setHalignment(HPos.CENTER);
		
		//Set the percent with for the thrid column to 50%
