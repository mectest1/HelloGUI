...
//		int a;
	...
	public void testFinally(){
		out.println(getFinallyResult());
	}
	
	/**
	 * <h4>Generated bytecodes:</h4>
	 * <code><pre>
    ...
    Code:
      ...
      LocalVariableTable:
        Start  Length  Slot  Name   Signature
            0      15     0  this   Lcom/mec/duke/LanguageFeatureTest;
            2      13     1     i   I
      StackMapTable: number_of_entries = 1
        frame_type = 255		//full frame
          offset_delta = 10
          locals = [ class com/mec/duke/LanguageFeatureTest, int ]
		...
		<h5>Funny fact</h5>
		<p>
		The opcode <code>jsr</code> is not even used here
		</p>
	 * @return
	 */
	public int getFinallyResult(){
	...
	public void testWait(){
		...
		Runnable r = () -> {
			synchronized(obj){
				try {
					out.printf("start waiting on \t%s\n", LocalDateTime.now());
					obj.wait(3000);		//<- wait until get notified, or time expires
				} catch (Exception e) {
					e.printStackTrace(out);
				}
				out.printf("end waiting on \t\t%s\n", LocalDateTime.now());
			}
		};
